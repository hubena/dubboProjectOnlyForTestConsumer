...
import server.AbstractAppInitializer;
import server.DefaultAppInitializer;
...
import core.netty.bootstrap.ServerBootstrap;
import core.netty.channel.Channel;
import core.netty.channel.ChannelFuture;
import core.netty.channel.ChannelOption;
import core.netty.channel.EventLoopGroup;
import core.netty.channel.nio.NioEventLoopGroup;
import core.netty.channel.socket.nio.NioServerSocketChannel;
import core.netty.handler.logging.LoggingHandler;
import core.netty.util.concurrent.DefaultThreadFactory;
 
public class Bserver {
 
	/** 托管系统原有参数设定值，超时15 seconds */
	public static final int TIME_OUT = 15 * 1000;
 
	/** 托管系统原有参数设定值,最大包长16M */
	public static final int DEFAULT_MAX_LEN = 16 * 1024 * 1024;
 
	private static final String SHUTDOWN = "SHUTDOWN";
 
	private static final String FLUSHLOG = "LOGFLUSH";
 
	public static void main(String args[]) {
		try {
			Thread.currentThread().setName("Bserver");
			if (args != null && args.length != 0) {
				String command = args[0];
				if (command.equalsIgnoreCase("STOP")) {
					stop();
				} else if (command.equalsIgnoreCase(FLUSHLOG)) {
					flush();
				} else {
					start();
				} 
			...
			log.error("Bserver run failed.", t);
			System.exit(1);
		}
